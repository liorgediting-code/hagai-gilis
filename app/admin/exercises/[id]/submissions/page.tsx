...
import type { FileUploadAnswer, FileUploadExercise, TextAnswerAnswer, TextAnswerExercise } from "@/lib/types/exercise-types";
...
export default async function ExerciseSubmissionsPage({ params }: Props) {
  ...
  const content = exercise.content_json as FileUploadExercise | TextAnswerExercise | null;
  if (!content || (content.type !== "file_upload" && content.type !== "text_answer")) notFound();
  const isTextExercise = content.type === "text_answer";
  ...
  // Sign each stored file for download (1h) — file-upload exercises only.
