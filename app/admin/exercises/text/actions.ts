...
import { textAnswerSchema } from "@/lib/types/exercise-types";
...
function buildContent(formData: FormData):
  | { ok: true; data: z.infer<typeof textAnswerSchema> }
  | { ok: false; error: string } {
  const parsed = textAnswerSchema.safeParse({
    type: "text_answer",
...
export async function createTextExerciseAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  await requireAdmin();
...
export async function updateTextExerciseAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  await requireAdmin();
